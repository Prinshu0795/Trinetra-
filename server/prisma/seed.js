...
exports.seedScenario = seedScenario;
// server/prisma/seed.ts
const client_1 = require("@prisma/client");
const bcryptjs_1 = __importDefault(require("bcryptjs"));
const prisma = new client_1.PrismaClient();
async function seedScenario(db = prisma) {
    ...
    const commonPasswordHash = await bcryptjs_1.default.hash('Password123!', salt);
    // 1. Seed Users
    ...
    // 2. Seed Primary Disaster Scenario: Brahmaputra River Flood (Kamrup Metropolitan, Assam)
    const disaster = await db.disaster.create({
        data: {
            ...
            longitude: 91.7362,
            radiusKm: 25.0,
            description: 'Water level in River Brahmaputra crossed highest danger level (51.5m). Severe inundation reported across low-lying wards with partial embankment failure near Pandu Ghat.',
            affectedPopulationEst: 140000,
            source: 'Central Water Commission (CWC) & ASDMA Operations Center',
            declaredAt: new Date(Date.now() - 6 * 3600 * 1000), // 6 hours ago
        },
    });
    ...
    console.log(`✓ Emergency Resources created: ${resourcesData.length}`);
    ...
    const reportsData = [
        {
            ...
            description: 'Sluice gate backflow is causing water to surge into residential lane #3. 4 elderly citizens stranded on rooftop.',
        ...
        },
        {
            ...
            description: 'Road culvert caved in under intense drainage overflow. Traffic fully blocked; emergency vehicles cannot pass.',
    ...
    console.log(`✓ Incident Reports created: ${reportsData.length}`);
    ...
    const alertsData = [
        {
            ...
            detailedMessage: 'Brahmaputra water level continues to rise at 4cm/hour. All residents within 500m of river embankment are ordered to evacuate immediately to designated safe shelters. Do not attempt to cross flooded underpasses.',
            actionInstructions: '1. Switch off main electricity breakers.\n2. Carry identification, essential medicines, and torch.\n3. Move immediately towards Nehru Stadium Relief Camp or Dispur Center.\n4. Call 1070 or 112 for NDRF water rescue.',
        ...
        },
        {
            ...
            detailedMessage: 'Regional Meteorological Centre forecasts intense thunderstorms (120-160mm) triggering localized hill landslides and flash flooding.',
            actionInstructions: 'Avoid hillslope settlements. Keep emergency phones charged. Store 3 days of potable drinking water.',
    ...
    console.log(`✓ Emergency Broadcast Alerts created: ${alertsData.length}`);
    ...
    await db.riskAssessment.create({
        data: {
            ...
            explanationText: 'Compound hazard analysis flags Guwahati urban core under severe flood emergency driven by high Brahmaputra discharge coupled with prolonged monsoon rainfall.',
            advisoryText: 'Enforce mandatory evacuation of low-lying floodplains. Keep all medical and NDRF rescue units on red alert.',
            isFallback: false,
        },
    });
    console.log('✓ Risk Assessment record created');
